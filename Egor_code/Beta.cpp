...
bool opros_players_about_new_word(string word, Statistics names, short player_ammount, short id) {
    {
        ...
        for (int i = 0; i < player_ammount; i++)
        {
            if (i != id)
            {
                cout << "Игрок " << names.names[i] << " согласны ли вы с тем что слово " << word << " должно войти в словарь и засчитаться игроку " << names.names[id] << " (да/нет): ";
                getline(cin, answer);
                if (answer == "да")
                {
                    count_of_plus += 1;
...
bool big_check(string user_word, Statistics names, short player_ammount, short id, Player player) {
    ...
    if (counter == user_word.length()) {
        fstream fin;
        string stroka{};
        if (user_word[0] >= 'а' && user_word[0] < 'з' || user_word[0] == 'ё') {

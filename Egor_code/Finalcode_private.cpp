...
struct Player
{
    ...
    short point = 0;
    string last_word_of_player{};
};

string lastly_played_word{"0"};
...
void main() {
    ...
    while (word != "0")
    {
       ...
       /* cout << endl << endl << "Изменённый алфавит: ";
        ...
        bool check = big_check(word, pl1);
        if (check)
        {
            scoring_of_players(check, word, lastly_played_word);
...
int scoring_of_players(bool correct_answer, string answer, string answer_previous_player)
{
    ...
    {
        if (answer_previous_player!="0" && answer[answer.length() - 1] == answer_previous_player[1])
